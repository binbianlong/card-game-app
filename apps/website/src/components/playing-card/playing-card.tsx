  ...
  return (
    ...
    >
      {faceDown ? (
        ...
        <CardFace isJoker={isJoker} meta={meta} rank={visibleRank} styles={styles} />
      )}
    </button>
  );
}
  ...
  return (
    <>
      <span className={cn("absolute top-2 left-2 grid justify-items-center font-black", meta.tone)}>
        <span className={cn("tabular-nums", styles.corner)}>{rank}</span>
      ...
      <span className={cn("grid place-items-center gap-1 self-center font-black", meta.tone)}>
        <span className={cn("leading-none tracking-normal", styles.centerRank)}>
          {isJoker ? "J" : rank}
        </span>
